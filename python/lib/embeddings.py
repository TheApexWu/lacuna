...
def compute_embedding_weight(
    ...
    """
    Compute semantic weight based on centrality (mean cosine similarity
    to all other concepts in the same language).

    High centrality = concept is well-connected = high weight (tall peak).
    Low centrality = peripheral/outlier concept = low weight.

    Note: L2 norm is useless here because embed_texts normalizes all
    embeddings to unit length.

    Args:
        embedding: Single embedding (already L2-normalized)
        all_embeddings: All embeddings in the set (already L2-normalized)
    ...
    # Cosine similarity to all others (since normalized, dot product = cosine)
    sims = (all_embeddings @ embedding.reshape(-1, 1)).flatten()

    # Exclude self-similarity (will be ~1.0)
    # Use mean of all OTHER similarities as centrality
    # Sort and take all but the highest (self)
    sorted_sims = np.sort(sims)[:-1]
    centrality = float(np.mean(sorted_sims))

    # Normalize centrality to 0-1 range across the batch
    all_centralities = []
    for i in range(len(all_embeddings)):
        s = (all_embeddings @ all_embeddings[i].reshape(-1, 1)).flatten()
        s_others = np.sort(s)[:-1]
        all_centralities.append(float(np.mean(s_others)))

    min_c = min(all_centralities)
    max_c = max(all_centralities)

    if max_c - min_c < 1e-6:
        return 0.5

    weight = (centrality - min_c) / (max_c - min_c)
    return float(np.clip(weight, 0.05, 0.95))
